...
class FacultyStaffListScreen extends StatefulWidget {
  const FacultyStaffListScreen({super.key});

  @override
  State<FacultyStaffListScreen> createState() => _FacultyStaffListScreenState();
}

class _FacultyStaffListScreenState extends State<FacultyStaffListScreen> {
  ...
  void fetchData() async {
    await facultyHomeController.fetchFacultyData();
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        if (facultyHomeController.isLoading.value) {
          return _buildShimmerEffect();
        }

        if (facultyHomeController.facultyList.isEmpty) {
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            itemCount: facultyHomeController.facultyList.length,
            padding: EdgeInsets.symmetric(vertical: 10.h),
            itemBuilder: (context, index) {
              FacultyModel faculty = facultyHomeController.facultyList[index];
              return Container(
